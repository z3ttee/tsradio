...
public class ChannelRepository extends Repository<Channel> {
    ...
    public ArrayList<Channel> findAll() {
        ...
        try {
            do {
                Channel channel = new Channel(rs.getString("uuid"), rs.getString("title"), rs.getString("path"), rs.getString("description"), rs.getBoolean("featured"));
    ...
    public Channel findOneByID(String uuid) {
        ...
        try {
            return new Channel(rs.getString("uuid"), rs.getString("title"), rs.getString("path"), rs.getString("description"), rs.getBoolean("featured"));
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
