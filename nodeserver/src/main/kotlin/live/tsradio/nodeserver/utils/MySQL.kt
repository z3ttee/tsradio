...
object MySQL {
    ...
    fun setup() {
        ...
        if(exists(tableNodes, "id = '${Filesystem.preferences.node.clientID}'")) {
            val map = HashMap<String, String>()
            map["id"] = Filesystem.preferences.node.clientID.toString()
            map["name"] = "NodeServer-${Filesystem.preferences.node.clientID.toString().subSequence(0,4)}"
            map["lastLogin"] = System.currentTimeMillis().toString()

            update(tableNodes, "id = '${Filesystem.preferences.node.clientID}'", map)
        } else {
            val map = HashMap<String, String>()
            map["id"] = Filesystem.preferences.node.clientID.toString()
            map["name"] = "NodeServer-${Filesystem.preferences.node.clientID.toString().subSequence(0,4)}"
    ...
    fun insert(table: String, fields: HashMap<String, String>): Boolean {
        return if(hasConnection()) {
            var keys = ""
            var values = ""
            for((index, entry) in fields.entries.withIndex()) {
