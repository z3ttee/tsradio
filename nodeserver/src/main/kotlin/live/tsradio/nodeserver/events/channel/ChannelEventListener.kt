...
object ChannelEventListener {
    ...
    fun onChannelStop(channel: Channel, reason: Int = REASON_SHUTDOWN_TRIGGERED){
        ...
        if(reason == REASON_CHANNEL_EXCEPTION && Filesystem.preferences.channels.autorestart) {
            // Restart channel
            var triesUsed = ChannelHandler.restartTries.getOrDefault(channel.data.id, 0)
            ...
            when {
                triesUsed == 1 -> {
                    ...
                    Thread.sleep(delay)
                    ChannelHandler.restartTries[channel.data.id] = triesUsed
                ...
                triesUsed > Filesystem.preferences.channels.restartTries -> {
                    ...
                    ChannelHandler.restartTries.remove(channel.data.id)
                }
                else -> {
                    // Execute with delay
        ...
        logger.info("Shut down: ${ChannelHandler.getChannel(channel.data.id)!!.isRunning}")
    }
